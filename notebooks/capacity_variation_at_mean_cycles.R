...
"
This specific notebook compares the distribution of retentions at mean cycles to
x%. This works in two steps:
1. Identify the mean cycles to x% for each group (baseline and fast)
2. Within each group, take a 'vertical slice' to determine the disribution of
   retentions.
   
This method is also known as the 'cut horizontally first, then vertically'
"
...
compute_statistics <- function(kable_in, temp_label) {
" 
...
"
   ...
   cv_test_asymptotic <- with(kable_in, 
                              asymptotic_test(retention_at_mean_cycles_to_80, 
                                              is_baseline_formation)
   )
   
   cv_test_MSLRT <- with(kable_in, 
                         mslr_test(nr = 1e4,
                                   retention_at_mean_cycles_to_80,
                                   is_baseline_formation))
   
   num_data_points = floor(lengths(kable_in['is_baseline_formation'])/2)
   
   agg = aggregate(kable_in, 
                   by = list(kable_in$is_baseline_formation), 
                   FUN = mean)
   
   
   label = sprintf("%s, comparison at mean cycles to 80%% \nD'AD Statistic: %.4f, p = %.4f, n = %g", 
                   temp_label,
   ...
   ggplot(kable_in,
          aes(is_baseline_formation, retention_at_mean_cycles_to_80)) +
      ...
      labs(subtitle = label,
           y = "Retention",
           x = NULL) + 
      scale_x_discrete(labels=c(sprintf("Fast, %g cycles",     agg[1, 'cycles_to_80_target_vec']),
                                sprintf("Baseline, %g cycles", agg[2, 'cycles_to_80_target_vec'])))
...
EndOfLife = read.csv('retention_at_mean_cycles.csv')
...
#compute_statistics(room_temp, 'Room temp') 
compute_statistics(high_temp, '45C') 
