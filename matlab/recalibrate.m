function [Un_modified, Up_modified] = recalibrate(Voltage, Q, Xt, Un, Up)
    ...
    % Assume that the beginning / exponential increase in voltage
    % region is only due to the graphite
    ...
    % Assume negative potential Up is correct and calculate Un
    Un_data = -Voltage + Up(y100 + (max(Q) - Q) ./ Cp);

    Xi = [0.063; -75; 0];

    % This is based from the original Un but with small tweaks to some of the
    % numbers to improve alignment to the actual dataset
    ...
    % Fit only the initial 100 data points from the Un; corresponds to the 
    % beginning (exponential) part of the curve.
    xtarg = xtarg_g(1:100);
    Vtarg = Un_data(1:100);
